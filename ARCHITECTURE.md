...
- Development starts Vite on port 5173 and the optional Node API on port 4173. Vite proxies `/api` to that server. The XAMPP build calls `api/index.php?route=...` directly.
...
The PHP API runs under the same origin as the React app. It exposes read, create/update, and delete operations for the repository methods. MySQL writes use PDO prepared statements and transactions. Imported photos are written under random filenames; the database stores only those generated names.

The PHP API enforces request-size limits for JSON, image, and backup files. Backup replacement validates the ZIP, manifest, structured records, and referenced images, then stages image files and replaces database rows in one MySQL transaction. If the database transaction fails, staged files are removed. Browser-origin data is migrated only once and only into an empty database.
