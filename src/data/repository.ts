...
async function performLegacyMigration() {
  ...
  if (status.empty) {
    ...
    if (hasRecords) {
      const archive = await makeArchive(legacy.data, legacy.blobs)
      await request('/api/import-legacy', { method: 'POST', headers: { 'content-type': 'application/zip' }, body: archive })
    }
  }
...
export const repository = {
  ...
  async getPhotoBlob(id: string) {
    const response = await fetch(`/api/photos/${encodeURIComponent(id)}/blob`, { cache: 'no-store' })
  ...
  async replaceAll(data: AppData, blobs: Map<string, Blob>) {
    ...
    await request('/api/replace', { method: 'POST', headers: { 'content-type': 'application/zip' }, body: archive })
  },
}
